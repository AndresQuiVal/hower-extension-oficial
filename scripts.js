...
document.addEventListener("DOMContentLoaded", function () {
  ...
  templates.forEach((template) => {
    template.addEventListener("click", function (e) {
      e.preventDefault();
      const message = this.querySelector("small i").innerText;
...
document
  ...
  .addEventListener("click", async function () {
    ...
    if (await HowerAPI.tokenNeedsValidation(howerUsername, howerToken)) {
      ...
      welcomeSectionPresentation.style.display = "block";
...
async function sendInstagramDMMessages() {
  ...
  if (isInspectingAndSending || isSending) {
    ...
    while (true) {
      ...
      await delay((1000 * 60) / 2); // waits half a minutes
...
class InstagramApi {
  ...
  //     const url = `${HOWER_TEST_ENDPOINT}/clients/login-insta/`;  // Adjust the URL if your endpoint is located elsewhere
  ...
  //       const response = await fetch(`${HOWER_TEST_ENDPOINT}/clients/api/make-request/`, {
  //           method: 'POST',
  //           headers: {
  //               'Content-Type': 'application/json',
  //           },
